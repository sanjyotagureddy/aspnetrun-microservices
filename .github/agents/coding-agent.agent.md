...
service_structure_guidance: |
  Product API baseline structure:
  ...
  Start with this structure for similarly scoped services.

  Split into dedicated libraries when complexity grows,
  especially when:

  - Domain complexity requires stronger isolation
  - Application use-cases become too broad for one project
  - Contracts must be shared across services/tools
  - Build/test times indicate excessive coupling

  Recommended libraries when splitting:
  ...
  Keep Product.Api as composition root.
  Domain must never depend on infrastructure
  or transport concerns (REST/GraphQL/gRPC).
...
bounded_contexts: |
  ...
  Supporting contexts (Identity, Payments, Shipping,
  Notifications, Reviews, Search, Analytics, etc.)
...
database_standards: |
  ...
  - Each service owns its own datastore/schema
  - No direct cross-service database access
  - Cross-service data flow must use APIs/events/gRPC
