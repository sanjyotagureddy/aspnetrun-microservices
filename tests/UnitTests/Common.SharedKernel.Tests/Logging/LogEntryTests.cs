...
public sealed class LogEntryTests
{
    ...
    {
        var entry = LogEntry.Create("test-service", "orders", "a","created", DateTimeOffset.UtcNow, "   ");
    ...
    {
        var entry = LogEntry.Create("test-service", "orders", "orders", "created", DateTimeOffset.UtcNow);
